 ...
 */
@IBDesignable
open class InxTextField: UITextField { // swiftlint:disable:this type_body_length
    /**
    ...
    open var errorMessagePlacement: ErrorMessagePlacement = .bottom {
        didSet {
            updateControl()
            updatePlaceholder()
        }
    ...
    */
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(InxTextField.editingChanged), for: .editingChanged)
    }
    ...
    */
    ...
    override open var isEnabled: Bool {
        didSet {
            ...
            updatePlaceholder()
            if isEnabled == false{
                self.font = UIFont.boldSystemFont(ofSize: self.font?.pointSize ?? 8)
            }
        }
    }
    ...
    */
    ...
    open func errorLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if errorMessagePlacement == .default {
            ...
            if editing {
                let originY = lineRect.origin.y + selectedLineHeight + 5
                return CGRect(x: 0, y: originY, width: bounds.size.width, height: errorHeight())
            }
            let originY = lineRect.origin.y + selectedLineHeight + 5 + errorHeight()
            return CGRect(x: 0, y: originY, width: bounds.size.width, height: errorHeight())
        }
    }
